...
document.addEventListener("DOMContentLoaded", () => {
  ...
  btn.addEventListener("click", async () => {
    const isMusicIcon = btn.classList.contains("--icon-music");
    try {
      if (isMusicIcon) {
        btn.classList.remove("--icon-music");
        btn.classList.add("play-music");
        btn.setAttribute("aria-pressed", "true");
        if (audio.readyState < 2) audio.load();
        await audio.play();
      } else {
        btn.classList.remove("play-music");
...
function boot() {
  ...
  miniClick?.addEventListener("click", (e) => {
    e.preventDefault();
...
document.querySelectorAll(".anim-title").forEach((el) => {
  ScrollTrigger.create({
    ...
    onEnter: () => {
      ...
      const words = new SplitText(el, { type: "words", wordsClass: "word++" }).words;
  ...
  const startPreloader = () => {
    ...
    const collect = () => {
      const vids = Array.from(document.querySelectorAll("video")).filter(
        (v) => v.src || v.getAttribute("src") || v.querySelector("source")
      );
...
(() => {
  ...
  function ensureTiltLayer(card) {
    let tilt = card.querySelector(".discover__tilt");
    if (!tilt) {
      tilt = document.createElement("div");
      tilt.className = "discover__tilt";
      while (card.firstChild) tilt.appendChild(card.firstChild);
  ...
  function initTilt(card) {
    ...
    const tilt = ensureTiltLayer(card);
